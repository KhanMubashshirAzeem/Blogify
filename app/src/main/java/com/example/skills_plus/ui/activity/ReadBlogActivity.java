...
import androidx.core.widget.NestedScrollView; // Updated Import
...
public class ReadBlogActivity extends AppCompatActivity {
  ...
  private void setupListeners() {
    // Changed to NavigationOnClickListener so only the back arrow triggers this
    binding.blogDetailToolbar.setNavigationOnClickListener(view -> onBackPressed());
    binding.downloadButton.setOnClickListener(v -> createPdf(binding.blogContentLayout));
  }

  /**
   * Creates a PDF from the provided NestedScrollView content and saves it to the device.
   * This is a UI-specific operation and can remain within the Activity.
   *
   * @param scrollView The NestedScrollView containing the content to be converted.
   */
  public void createPdf(NestedScrollView scrollView) { // Updated Parameter Type
